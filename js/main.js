var _sendError = function(e, name) {
    e = e || {name:'UndefinedError', message:''};
    var action = document.uniqueID && document.documentMode ? 'ie'+document.documentMode : 'browser';
    var label = location.href+': '+e.name+': '+e.message+(e.stack ? ': '+e.stack : '')+' ['+navigator.userAgent+']';
    ga('send', 'event', 'error', action+' '+name, label, 1, {nonInteraction:true});
  };
